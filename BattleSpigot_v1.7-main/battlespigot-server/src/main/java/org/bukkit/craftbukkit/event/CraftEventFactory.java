...
import dev.nulledcode.spigot.events.AirdropDieEvent;
import dev.nulledcode.spigot.util.StackerUtils;
...
public class CraftEventFactory {
    ...
    private static boolean canBuild(CraftWorld world, Player player, int x, int z) {
        ...
        int distanceFromSpawn = FastMath.max(FastMath.abs(x - chunkcoordinates.x), FastMath.abs(z - chunkcoordinates.z));
    ...
    public static CreatureSpawnEvent callCreatureSpawnEvent(EntityLiving entityliving, SpawnReason spawnReason) {
        ...
        if (SpigotConfig.mobstacker) {
            int radius = 12;
            entity.getNearbyEntities(radius, radius, radius).forEach(nearby -> StackerUtils.stackEntity(nearby, entity));
        }
    ...
    public static EntityDeathEvent callEntityDeathEvent(EntityLiving victim, List<org.bukkit.inventory.ItemStack> drops, int exp) {
        ...
        if (SpigotConfig.mobstacker) {
            if (event.getEntity().getType() != EntityType.PLAYER) {
                StackerUtils.unstackEntity(event.getEntity());
            }
        }
        ...
        if (entity.getKiller() != null && entity.getKiller() instanceof Player) {
            ...
            for (org.bukkit.inventory.ItemStack stack : drops) {
                ...
                if (killer.getInventory().firstEmpty() == -1 || entity.hasMetadata("CombatLogger") || entity.hasMetadata("loggerBait")) {
                    world.dropItemNaturally(entity.getLocation(), stack);
                } else {
                    for (org.bukkit.inventory.ItemStack item2 : event.getDrops()) {
                        if (item2 != null && item2.getType() != Material.AIR) {
                            // bSpigot Start
                            if(SpigotConfig.disableItemDrops) {
                                item2 = item2.clone();
                                for (final Map.Entry<Integer, org.bukkit.inventory.ItemStack> excess : killer.getInventory().addItem(new org.bukkit.inventory.ItemStack[]{item2.clone()}).entrySet()) {
                                    world.dropItemNaturally(entity.getLocation(), excess.getValue());
                                }
                            } else {
                                world.dropItemNaturally(entity.getLocation(), item2);
                            }
                            // bSpigot - End
                        }
                    }
    ...
    public static AirdropDieEvent callAirdropDieEvent(org.bukkit.entity.Entity entity) {
        AirdropDieEvent event = new AirdropDieEvent(entity);
        entity.getServer().getPluginManager().callEvent(event);
        return event;
    }
    ...
    public static ProjectileHitEvent callProjectileHitEvent(Entity entity, Entity target) {
        ProjectileHitEvent event = new ProjectileHitEvent((Projectile) entity.getBukkitEntity(), target == null ? null : target.getBukkitEntity());
