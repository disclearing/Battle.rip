...
public abstract class EntityHanging extends Entity {
    ...
    public boolean survives() {
        if (!this.world.getCubes(this, this.boundingBox).isEmpty()) {
            ...
            int i = FastMath.max(1, this.f() / 16);
            int j = FastMath.max(1, this.i() / 16);
            int k = this.x;
            int l = this.y;
            int i1 = this.z;

            if (this.direction == 2) {
                k = (int) FastMath.floor(this.locX - (double) ((float) this.f() / 32.0F));
            }

            if (this.direction == 1) {
                i1 = (int) FastMath.floor(this.locZ - (double) ((float) this.f() / 32.0F));
            }
            ...
            l = (int) FastMath.floor(this.locY - (double) ((float) this.i() / 32.0F));
