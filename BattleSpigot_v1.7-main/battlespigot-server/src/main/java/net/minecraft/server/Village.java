...
public class Village {
    ...
    public VillageDoor e(int i, int j, int k) {
        if (this.center.e(i, j, k) > (float) (this.size * this.size)) {
            ...
            } while (villagedoor.locX != i || villagedoor.locZ != k || FastMath.abs(villagedoor.locY - j) > 1);
    ...
    private void l() {
        ...
        while (iterator.hasNext()) {
            ...
            if (!villageaggressor.a.isAlive() || FastMath.abs(this.time - villageaggressor.b) > 300) {
                iterator.remove();
            }
        }
    }
    ...
    private void m() {
        ...
        while (iterator.hasNext()) {
            ...
            if (!this.isDoor(villagedoor.locX, villagedoor.locY, villagedoor.locZ) || FastMath.abs(this.time - villagedoor.addedTime) > 1200) {
                this.c.x -= villagedoor.locX;
                this.c.y -= villagedoor.locY;
                this.c.z -= villagedoor.locZ;
                flag = true;
    ...
    private void n() {
        ...
        if (i == 0) {
            ...
            for (Iterator iterator = this.doors.iterator(); iterator.hasNext(); j = FastMath.max(villagedoor.b(this.center.x, this.center.y, this.center.z), j)) {
                villagedoor = (VillageDoor) iterator.next();
            }

            this.size = FastMath.max(32, (int) FastMath.sqrt((double) j) + 1);
        }
