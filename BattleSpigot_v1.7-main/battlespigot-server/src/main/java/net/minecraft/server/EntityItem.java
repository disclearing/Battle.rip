...
public class EntityItem extends Entity {
    ...
    public EntityItem(World world, double d0, double d1, double d2) {
        super(world);
        this.e = 5;
        this.c = (float) (FastMath.random() * 3.141592653589793D * 2.0D);
        this.a(0.25F, 0.25F);
    ...
    public void h() {
        if (this.getItemStack() == null) {
            ...
            if (flag || this.ticksLived % 25 == 0) {
                if (this.world.getType((int) FastMath.floor(this.locX), (int) FastMath.floor(this.locY), (int) FastMath.floor(this.locZ)).getMaterial() == Material.LAVA) {
                    this.motY = 0.20000000298023224D;
            ...
            if (this.onGround) {
                f = this.world.getType((int) FastMath.floor(this.locX), (int) FastMath.floor(this.boundingBox.b) - 1, (int) FastMath.floor(this.locZ)).frictionFactor * 0.98F;
            }
            ...
            if (this.world.spigotConfig.altHopperTicking) {
                int xi = (int) FastMath.floor(this.boundingBox.a);
                int yi = (int) FastMath.floor(this.boundingBox.b) - 1;
                int zi = (int) FastMath.floor(this.boundingBox.c);
                int xf = (int) FastMath.floor(this.boundingBox.d);
                int yf = (int) FastMath.floor(this.boundingBox.e) - 1;
                int zf = (int) FastMath.floor(this.boundingBox.f);
                for (int a = xi; a <= xf; a++) {
                    for (int c = zi; c <= zf; c++) {
                        for (int b = yi; b <= yf; b++) {
