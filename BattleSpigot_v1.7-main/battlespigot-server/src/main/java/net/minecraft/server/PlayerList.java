...
public abstract class PlayerList {
    ...
    private final Map<UUID, ServerStatisticManager> n;
    ...
    public void a(EntityPlayer entityplayer, WorldServer worldserver) {
        ...
        worldserver1.chunkProviderServer.getChunkAt((int) FastMath.floor(entityplayer.locX) >> 4, (int) FastMath.floor(entityplayer.locZ) >> 4);
    }
    ...
    protected void b(EntityPlayer entityplayer) {
        ...
        ServerStatisticManager serverstatisticmanager = this.n.get(entityplayer.getUniqueID());
    ...
    public void c(EntityPlayer entityplayer) {
        ...
        // CraftBukkit start
        PlayerJoinEvent playerJoinEvent = new PlayerJoinEvent(this.cserver.getPlayer(entityplayer), null);
        ...
        PacketPlayOutPlayerInfo removePacket = PacketPlayOutPlayerInfo.removePlayer( entityplayer );
        for (int i = 0; i < this.players.size(); ++i) {
            EntityPlayer entityplayer1 = (EntityPlayer) this.players.get(i);
            boolean is18 = entityplayer1.playerConnection.networkManager.getVersion() > 28;
            ...
            if (entityplayer1.getBukkitEntity().canSee(entityplayer.getBukkitEntity())) {
                ...
                //if (is18 && SpigotConfig.onlyCustomTab) entityplayer1.playerConnection.sendPacket(removePacket);
                // Spigot end
            }
        }
        // CraftBukkit end
        ...
        for (int i = 0; i < this.players.size(); ++i) {
            ...
            //if (is18 && SpigotConfig.onlyCustomTab) entityplayer.playerConnection.sendPacket(PacketPlayOutPlayerInfo.removePlayer(entityplayer1));
            // Spigot end
            // CraftBukkit end
        }
    }
    ...
    public String disconnect(EntityPlayer entityplayer) { // CraftBukkit - return string
        ...
        PlayerQuitEvent playerQuitEvent = new PlayerQuitEvent(this.cserver.getPlayer(entityplayer), null);
    ...
    public EntityPlayer attemptLogin(LoginListener loginlistener, GameProfile gameprofile, String hostname) {
        ...
        EntityPlayer entity = new EntityPlayer(this.server, this.server.getWorldServer(0), gameprofile, new PlayerInteractManager(this.server.getWorldServer(0)), true);
    ...
    public EntityPlayer moveToWorld(EntityPlayer entityplayer, int i, boolean flag, Location location, boolean avoidSuffocation) {
        ...
        worldserver.chunkProviderServer.getChunkAt((int) FastMath.floor(entityplayer1.locX) >> 4, (int) FastMath.floor(entityplayer1.locZ) >> 4);
    ...
    public List a(ChunkCoordinates chunkcoordinates, int i, int j, int k, int l, int i1, int j1, Map map, String s, String s1, World world) {
        if (this.players.isEmpty()) {
            ...
            if (k > 0) {
                object = ((List) object).subList(0, FastMath.min(k, ((List) object).size()));
            }
    ...
    public ServerStatisticManager a(EntityHuman entityhuman) {
        ...
        ServerStatisticManager serverstatisticmanager = uuid == null ? null : this.n.get(uuid);

        if (serverstatisticmanager == null) {
            File file1 = new File(this.server.getWorldServer(0).getDataManager().getDirectory(), "stats");
            File file2 = new File(file1, FastUUID.toString(uuid) + ".json");
