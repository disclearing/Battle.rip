...
public class Pathfinder {
    ...
    private PathEntity a(Entity entity, double d0, double d1, double d2, float f) {
        ...
        this.c.c();
        boolean flag = this.g;
        int i = (int) FastMath.floor(entity.boundingBox.b + 0.5D);

        if (this.h && entity.M()) {
            i = (int) entity.boundingBox.b;

            for (Block block = this.a.getType((int) FastMath.floor(entity.locX), i, (int) FastMath.floor(entity.locZ)); block == Blocks.WATER || block == Blocks.STATIONARY_WATER; block = this.a.getType((int) FastMath.floor(entity.locX), i, (int) FastMath.floor(entity.locZ))) {
                ++i;
            }
            ...
            i = (int) FastMath.floor(entity.boundingBox.b + 0.5D);
        }

        PathPoint pathpoint = this.a((int) FastMath.floor(entity.boundingBox.a), i, (int) FastMath.floor(entity.boundingBox.c));
        PathPoint pathpoint1 = this.a((int) FastMath.floor(d0 - (double) (entity.width / 2.0F)), (int) FastMath.floor(d1), (int) FastMath.floor(d2 - (double) (entity.width / 2.0F)));
            ...
            for (int i1 = j; i1 < j + pathpoint.b; ++i1) {
                for (int j1 = k; j1 < k + pathpoint.c; ++j1) {
                    ...
                    if (block.getMaterial() != Material.AIR) {
                        ...
                        if (k1 == 9) { // Poweruser
                            int l1 = (int) FastMath.floor(entity.locX);
                            int i2 = (int) FastMath.floor(entity.locY);
                            int j2 = (int) FastMath.floor(entity.locZ);
