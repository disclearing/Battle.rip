...
import dev.nulledcode.spigot.util.ItemPickupUtils;
...
public class PlayerInteractManager {
        ...
        if (false && this.gamemode.isAdventure() && !this.player.d(i, j, k)) { // Never trigger
            ...
            if (this.isCreative()) {
                ...
                if (flag && flag1) {
                    ...
                    if(SpigotConfig.disableItemDrops) { // bSpigot
                        if(player.getBukkitEntity().getInventory().firstEmpty() == -1) {
                            dropItem(i, j, k, items);
                        } else {
                            for (org.bukkit.entity.Item item : items) {
                                org.bukkit.inventory.ItemStack finaldrop = ItemPickupUtils.handleDrops(player.getBukkitEntity(), item.getItemStack());
                                if (finaldrop != null) {
                                    if (finaldrop.getType() != org.bukkit.Material.COBBLESTONE) {
                                        player.getBukkitEntity().getInventory().addItem(finaldrop);
                                    } else {
                                        if (player.getBukkitEntity().isPickingCobble())
                                            player.getBukkitEntity().getInventory().addItem(finaldrop);
                                    }
                                }
                            }
                    ...
                    if (flag && event != null) {
                        player.getBukkitEntity().giveExp(event.getExpToDrop());
                    }
                    // lSpigot end
                }
            }
    ...
    private void dropItem(int i, int j, int k, List<org.bukkit.entity.Item> items) {
        BlockDropItemsEvent dropItemsEvent = new BlockDropItemsEvent(world.getWorld().getBlockAt(i, j, k), player.getBukkitEntity(), items);
        ...
        if (!dropItemsEvent.isCancelled() && dropItemsEvent.getToDrop() != null) {
            for (final org.bukkit.entity.Item item : dropItemsEvent.getToDrop()) {
                if (item.getItemStack().getType() == org.bukkit.Material.COBBLESTONE && !player.getBukkitEntity().isPickingCobble()) continue;

                org.bukkit.inventory.ItemStack finalitem = ItemPickupUtils.handleDrops(player.getBukkitEntity(), item.getItemStack());
                item.setItemStack(finalitem);
