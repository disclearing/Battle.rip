...
import dev.nulledcode.spigot.flushing.FlushAPI;
...
public class NetworkManager extends SimpleChannelInboundHandler {
    ...
    public void handle(Packet packet, GenericFutureListener... agenericfuturelistener) {
        ...
        if (packet instanceof PacketPlayInUseEntity && ((PacketPlayInUseEntity) packet).c() == EnumEntityUseAction.ATTACK) {
            BattleSpigot.getInstance().getThread().getAsyncHitDetection().addPacket(packet, this, (net.minecraft.util.io.netty.util.concurrent.GenericFutureListener<? extends Future<? super Void>>[]) agenericfuturelistener);
            return;
        }

        if (this.isMovementPacket(packet)) {
            BattleSpigot.getInstance().getThread().getAsyncKnockbackDetection().addPacket(packet, this, (net.minecraft.util.io.netty.util.concurrent.GenericFutureListener<? extends Future<? super Void>>[]) agenericfuturelistener);
            return;
        }

        if (this.isBlockDigOrPlace(packet)) {
            BattleSpigot.getInstance().getThread().getTickThread().addPacket(packet, this, (net.minecraft.util.io.netty.util.concurrent.GenericFutureListener<? extends Future<? super Void>>[]) agenericfuturelistener);
            return;
        }
    ...
    private boolean isMovementPacket(Packet packet) {
        if (packet instanceof PacketPlayInFlying) return true;
        return false;
    }
