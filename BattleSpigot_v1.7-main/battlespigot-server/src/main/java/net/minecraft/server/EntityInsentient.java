...
import dev.nulledcode.spigot.util.pathsearch.AsyncNavigation;
...
public abstract class EntityInsentient extends EntityLiving {
    ...
    protected void w() {
        ...
        if (this.persistent) {
            ...
            // MineHQ start
            if (this.world.hardDespawnDistance == -1D) this.world.hardDespawnDistance = FastMath.sqrt(this.world.paperSpigotConfig.hardDespawnDistance);
    ...
    protected void bn() {
        ...
        this.world.methodProfiler.b();
        // Spigot Start
        if (SpigotConfig.DISABLED_AI_TYPES.contains(this.getBukkitEntity().getType()) || SpigotConfig.disablemobai && !this.getBukkitEntity().hasMetadata("enableai")) {
            return;
        }
    ...
    public void a(Entity entity, float f, float f1) {
        ...
        double d3 = (double) (int) FastMath.sqrt(d0 * d0 + d1 * d1);
        float f2 = (float) (FastMath.atan2(d1, d0) * 180.0D / 3.1415927410125732D) - 90.0F;
        float f3 = (float) (-(FastMath.atan2(d2, d3) * 180.0D / 3.1415927410125732D));
        ...
        for (int j = 0; j < this.getEquipment().length; ++j) {
            ...
            if (itemstack != null && (flag || flag1) && this.random.nextFloat() - (float) i * 0.01F < this.dropChances[j]) {
                if (!flag1 && itemstack.g()) {
                    int k = FastMath.max(itemstack.l() - 25, 1);
