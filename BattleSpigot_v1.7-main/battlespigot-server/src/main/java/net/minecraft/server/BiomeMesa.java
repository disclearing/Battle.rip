...
public class BiomeMesa extends BiomeBase {
    ...
    public void a(World world, Random random, Block[] ablock, byte[] abyte, int i, int j, double d0) {
        ...
        if (this.aH) {
            k = (i & -16) + (j & 15);
            l = (j & -16) + (i & 15);
            double d2 = FastMath.min(FastMath.abs(d0), this.aE.a((double) k * 0.25D, (double) l * 0.25D));

            if (d2 > 0.0D) {
                double d3 = 0.001953125D;
                double d4 = FastMath.abs(this.aF.a((double) k * d3, (double) l * d3));
        ...
        boolean flag1 = FastMath.cos(d0 / 3.0D * 3.141592653589793D) > 0.0D;
        int j1 = -1;
        boolean flag2 = false;
    ...
    private byte d(int i, int j, int k) {
        int l = (int) FastMath.round(this.aG.a((double) i * 1.0D / 512.0D, (double) i * 1.0D / 512.0D) * 2.0D);
