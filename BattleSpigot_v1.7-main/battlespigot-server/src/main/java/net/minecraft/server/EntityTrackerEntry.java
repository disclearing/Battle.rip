...
public class EntityTrackerEntry {
    ...
    public EntityTrackerEntry(EntityTracker entityTracker, Entity entity, int i, int j, boolean flag) { // MineHQ
        ...
        } else if (this.tracker instanceof EntityFishingHook || this.tracker instanceof EntityArrow || this.tracker instanceof EntityProjectile) {
    ...
    private boolean withinNoTrack(Entity entity) {
        ...
        if (player != null && player.hasMetadata("deathban")) return false;

        if (SpigotConfig.notrackbypass) {
            if (player != null && player.hasPermission("notrack.bypass") && !player.hasMetadata("invisible")) return false;
        }
    ...
    public void track(List list) {
        ...
        if (this.tracker instanceof EntityItemFrame /*&& this.m % 10 == 0*/) { // CraftBukkit - Moved below, should always enter this block
            ...
            if (this.tracker.vehicle == null) {
                ...
                j = (int) FastMath.floor(this.tracker.locY * 32.0D);
                ...
                if (this.u) {
                    ...
                    if (d4 > d3 * d3 || d4 > 0.0D && this.tracker.motX == 0.0D && this.tracker.motY == 0.0D && this.tracker.motZ == 0.0D) {
                        ...
                        if (this.tracker instanceof EntityFishingHook || this.tracker instanceof EntityArrow || this.tracker instanceof EntityProjectile) {
    ...
    public int getRange() {
        ...
        return FastMath.max(this.b, org.spigotmc.TrackingRange.getEntityTrackingRange(this.tracker.passenger, 0));
    }
    // MineHQ end
}