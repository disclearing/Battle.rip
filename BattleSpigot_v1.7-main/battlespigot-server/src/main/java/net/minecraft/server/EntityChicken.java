...
public class EntityChicken extends EntityAnimal {
    ...
    protected void dropDeathLoot(boolean flag, int i) {
        // bSpigot start - UHC has a different drop handling with scenarios
        // so just add a quick check if its uhc, then handle it as we want.
        ...
        if(!SpigotConfig.uhc) {
            ...
            int j = 1 + this.random.nextInt( 1 + i);

            if(random.nextBoolean()) this.a(Items.FEATHER, 1);
            this.a(isBurning() ? Items.COOKED_CHICKEN : Items.RAW_CHICKEN, j);
        }
    ...
    public void ac() {
        super.ac();
        float f = (float) FastMath.sin(this.aM * 3.1415927F / 180.0F);
        float f1 = (float) FastMath.cos(this.aM * 3.1415927F / 180.0F);
        float f2 = 0.1F;
        float f3 = 0.0F;
