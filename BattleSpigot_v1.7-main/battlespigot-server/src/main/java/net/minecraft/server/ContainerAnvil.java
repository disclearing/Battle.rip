...
public class ContainerAnvil extends Container {
    ...
    private final IInventory h = new ContainerAnvilInventory(this, "Repair", true, 2);
    ...
    public ContainerAnvil(PlayerInventory playerinventory, World world, int i, int j, int k, EntityHuman entityhuman) {
        ...
        this.a(new SlotAnvilResult(this, this.g, 2, 134, 47, world, i, j, k));
        ...
        if (itemstack == null) {
            ...
            if (itemstack2 != null) {
                ...
                if (itemstack1.g() && itemstack1.getItem().a(itemstack, itemstack2)) {
                    l = FastMath.min(itemstack1.j(), itemstack1.l() / 4);
                    if (l <= 0) {
                        this.g.setItem(0, null);
                        this.a = 0;
                        return;
                    ...
                    for (i1 = 0; l > 0 && i1 < itemstack2.count; ++i1) {
                        ...
                        i += FastMath.max(1, l / 100) + map.size();
                        l = FastMath.min(itemstack1.j(), itemstack1.l() / 4);
                    }
            ...
            for (iterator = map.keySet().iterator(); iterator.hasNext(); k += l + k1 * l1) {
                ...
                if (flag) {
                    l1 = FastMath.max(1, l1 / 2);
                }
            }

            if (flag) {
                k = FastMath.max(1, k / 2);
            }
            ...
            AnvilPreRepairEvent preRepairEvent = new AnvilPreRepairEvent(this.bukkitEntity, CraftItemStack.asBukkitCopy(itemstack1), this.a);
            Bukkit.getPluginManager().callEvent(preRepairEvent);

            if (itemstack1 != null && preRepairEvent.getCost() != itemstack1.getRepairCost()) this.a = preRepairEvent.getCost();
