...
public abstract class BlockDiodeAbstract extends BlockDirectional {
    ...
    public void a(World world, int i, int j, int k, Random random) {
        if (BattleSpigot.isPositionOfTileEntityInUse(world, i, j, k)) {
    ...
    protected int h(World world, int i, int j, int k, int l) {
        ...
        return l1 >= 15 ? l1 : FastMath.max(l1, world.getType(j1, j, k1) == Blocks.REDSTONE_WIRE ? world.getData(j1, j, k1) : 0);
    }
    ...
    protected int h(IBlockAccess iblockaccess, int i, int j, int k, int l) {
        ...
        case 0:
        case 2:
            return FastMath.max(this.i(iblockaccess, i - 1, j, k, 4), this.i(iblockaccess, i + 1, j, k, 5));

        case 1:
        case 3:
            return FastMath.max(this.i(iblockaccess, i, j, k + 1, 3), this.i(iblockaccess, i, j, k - 1, 2));
    ...
    public void postPlace(World world, int i, int j, int k, EntityLiving entityliving, ItemStack itemstack) {
        int l = (((int) FastMath.floor((double) (entityliving.yaw * 4.0F / 360.0F) + 0.5D) & 3) + 2) % 4;
