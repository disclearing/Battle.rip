...
public class EntitySlime extends EntityInsentient implements IMonster {
    ...
    public void h() {
        ...
        if (this.onGround && !flag) {
            ...
            for (int j = 0; j < i * 8; ++j) {
                ...
                float f3 = (float) (FastMath.cos(f) * (float) i * 0.5F * f1);
    ...
    public boolean canSpawn() {
        Chunk chunk = this.world.getChunkAtWorldCoords((int) FastMath.floor(this.locX), (int) FastMath.floor(this.locZ));
        ...
        if (this.world.getWorldData().getType() == WorldType.FLAT && this.random.nextInt(4) != 1) {
            ...
            if (this.getSize() == 1 || this.world.difficulty != EnumDifficulty.PEACEFUL) {
                ...
                if (biomebase == BiomeBase.SWAMPLAND && this.locY > 50.0D && this.locY < 70.0D && this.random.nextFloat() < 0.5F && this.random.nextFloat() < this.world.y()) {
