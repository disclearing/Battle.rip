...
public class Chunk {
    ...
    public ChunkMap getChunkMap(boolean groundUpContinuous, int primaryBitMask, int version) {
        if (!false || !groundUpContinuous || (primaryBitMask != 0 && primaryBitMask != '\uffff')) {
    ...
    public <T extends Entity> void a(Class<? extends T> oclass, AxisAlignedBB axisalignedbb, List<Entity> list, IEntitySelector ientityselector) {
        int i = MathHelper.floor((axisalignedbb.b - 2.0D) / 16.0D);
        int j = MathHelper.floor((axisalignedbb.e + 2.0D) / 16.0D);
