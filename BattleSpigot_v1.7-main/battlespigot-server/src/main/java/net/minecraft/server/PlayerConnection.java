...
public class PlayerConnection implements PacketPlayInListener {
    ...
    public void disconnect(String s) {
        ...
        PlayerKickEvent event = new PlayerKickEvent(this.server.getPlayer(this.player), s, null);
    ...
    public void a(PacketPlayInFlying packetplayinflying) {
        ...
        if (!this.player.viewingCredits) {
            ...
            if (packetplayinflying.hasPos && delta > 0.0D && this.checkMovement && !this.player.dead) {
                for (MovementHandler handler : BattleSpigot.INSTANCE.getMovementHandlers()) {
                    try {
            ...
            if ((delta > 1f / 256 || deltaAngle > 10f) && (this.checkMovement && !this.player.dead)) {
                ...
                if (!SpigotConfig.disablePlayerMoveEvent) { // Spigot - don't skip any move events
                    //this.getPlayer().getHandle().attack(getPlayer().getHandle());
            ...
            if (this.checkMovement && !this.player.dead) {
                ...
                if (packetplayinflying.j()) {
                    ...
                    if (FastMath.abs(packetplayinflying.c()) > 3.2E7D || FastMath.abs(packetplayinflying.e()) > 3.2E7D) {
                ...
                double d7 = FastMath.max(FastMath.abs(xDelta), FastMath.abs(this.player.motX));
                double d8 = FastMath.max(FastMath.abs(yDelta), FastMath.abs(this.player.motY));
                double d9 = FastMath.max(FastMath.abs(zDelta), FastMath.abs(this.player.motZ));
                // CraftBukkit end
                ...
                if (positionOffset > org.spigotmc.SpigotConfig.movedWronglyThreshold && !this.player.isSleeping()) {
                    ...
                    this.offsetDistanceSum += FastMath.sqrt(d10);
                }*/
                // Poweruser end
    ...
    private AxisAlignedBB getBoundingBoxRounded() {
        ...
        bb.a = (FastMath.round(bb.a * 1000000.0D) / 1000000.0D);
        bb.b = (FastMath.round(bb.b * 1000000.0D) / 1000000.0D);
        bb.c = (FastMath.round(bb.c * 1000000.0D) / 1000000.0D);
        bb.d = (FastMath.round(bb.d * 1000000.0D) / 1000000.0D);
        bb.e = (FastMath.round(bb.e * 1000000.0D) / 1000000.0D);
        bb.f = (FastMath.round(bb.f * 1000000.0D) / 1000000.0D);
        return bb;
    }
        ...
        if (packetplayinblockdig.g() == 4) {
            ...
            if (packetplayinblockdig.g() == 0) {
                if (!this.minecraftServer.a(worldserver, i, j, k, this.player)) {
                    ...
                    if (!SpigotConfig.disableLeftClickAirEvent) {
    ...
    public void handleKeepAliveSync(PacketPlayInKeepAlive packet)  {
        ...
        Iterator<Long> it = this.velocitySentTimes.iterator();
        int i = 0;
        while (it.hasNext()) {
            long ts = it.next();
    ...
    public void a(PacketPlayInWindowClick packetplayinwindowclick) {
        ...
        if (this.player.activeContainer.windowId == packetplayinwindowclick.c() && this.player.activeContainer.c(this.player)) {
            ...
            if (ItemStack.matches(packetplayinwindowclick.g(), itemstack)) {
                ...
                ArrayList<ItemStack> arraylist = new ArrayList<>(this.player.activeContainer.c.size()); // Velt
    ...
    public void a(PacketPlayInTabComplete packetplayintabcomplete) {
        BattleSpigot.getInstance().getThread().requestRunnable(() -> {
            Future<List<String>> future = (Future<List<String>>) BattleSpigot.getInstance().getThread().requestTask(this.possibleTabCompletions(packetplayintabcomplete));

            try {
                List<String> arraylist = future.get(50, TimeUnit.MILLISECONDS);
                ...
                future.cancel(true);
                this.player.getBukkitEntity().sendMessage("§4§l! §cYour tab completion took too long! Please report this to a staff member!");

                if(this.blockedAttempts++ >= 3) {
                    this.networkManager.lockDownIncomingTraffic();
                    this.minecraftServer.warning(this.player.getName() + " tried to crash the server (3x / TAB_COMPLETION)");
                }
            } catch (InterruptedException | ExecutionException ex) {}
        });
    }

    private Callable<List<String>> possibleTabCompletions(PacketPlayInTabComplete packet) {
        return new Callable<List<String>>() {
            @Override
            public List<String> call() throws Exception {
                List<String> list = new ArrayList<>();
                Iterator iterator = minecraftServer.a(player, packet.c()).iterator();
