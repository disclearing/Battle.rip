...
import dev.nulledcode.spigot.util.RemoveAllFix.Marker;
import dev.nulledcode.spigot.util.ThreadingManager;
...
public abstract class Entity implements Marker {
    ...
    public void C() {
        ...
        if (this.isSprinting() && !this.M()) {
            ...
            i = (int) FastMath.floor(this.locY - 0.20000000298023224D - (double) this.height);
    ...
    public void loadChunks() {
        int xstart = (int) FastMath.floor(this.locX);
        int zstart = (int) FastMath.floor(this.locZ);
        int xend = (int) FastMath.floor(this.locX + this.motX);
        int zend = (int) FastMath.floor(this.locZ + this.motZ);

        int xmin = FastMath.min(xstart, xend) - 3;
        int xmax = FastMath.max(xstart, xend) + 3;
        int zmin = FastMath.min(zstart, zend) - 3;
        int zmax = FastMath.max(zstart, zend) + 3;
    ...
    public void move(double d0, double d1, double d2) {
        ...
        if (this.X) {
            ...
            if ((this.positionChanged) && (this.getBukkitEntity() instanceof Vehicle)) {
                ...
                org.bukkit.block.Block block = this.world.getWorld().getBlockAt((int) FastMath.floor(this.locX), (int) FastMath.floor(this.locY - (double) this.height), (int) FastMath.floor(this.locZ));
            ...
            if (this.g_() && !flag && this.vehicle == null) {
                int l = (int) FastMath.floor(this.locX);

                k = (int) FastMath.floor(this.locY - 0.20000000298023224D - (double) this.height);
                int i1 = (int) FastMath.floor(this.locZ);
                ...
                this.P = (float) ((double) this.P + (double) FastMath.sqrt(d10 * d10 + d12 * d12) * 0.6D);
                this.Q = (float) ((double) this.Q + (double) FastMath.sqrt(d10 * d10 + d11 * d11 + d12 * d12) * 0.6D);
    ...
    protected void I() {
        int i = (int) FastMath.floor(this.boundingBox.a + 0.001D);
        int j = (int) FastMath.floor(this.boundingBox.b + 0.001D);
        int k = (int) FastMath.floor(this.boundingBox.c + 0.001D);
        int l = (int) FastMath.floor(this.boundingBox.d - 0.001D);
        int i1 = (int) FastMath.floor(this.boundingBox.e - 0.001D);
        int j1 = (int) FastMath.floor(this.boundingBox.f - 0.001D);
    ...
    private boolean withinNoTrack() {
        double xDistSqrd = locX * locX;
        double zDistSqrd = locZ * locZ;

        EntityTracker tracker = ((WorldServer) this.world).tracker;

        int noTrackDistanceSqrd = tracker.getNoTrackDistance() * tracker.getNoTrackDistance();
    ...
    public boolean L() {
        return this.inWater || this.world.isRainingAt((int) FastMath.floor(this.locX), (int) FastMath.floor(this.locY), (int) FastMath.floor(this.locZ)) || this.world.isRainingAt((int) FastMath.floor(this.locX), (int) FastMath.floor(this.locY + (double) this.length), (int) FastMath.floor(this.locZ));
    }
    ...
    public boolean N() {
        if (this.world.a(this.boundingBox.grow(0.0D, -0.4000000059604645D, 0.0D).shrink(0.001D, 0.001D, 0.001D), Material.WATER, this)) {
            if (!this.inWater && !this.justCreated) {
                float f = (int) FastMath.sqrt(this.motX * this.motX * 0.20000000298023224D + this.motY * this.motY + this.motZ * this.motZ * 0.20000000298023224D) * 0.2F;
                ...
                float f1 = (float) FastMath.floor(this.boundingBox.b);
    ...
    public boolean a(Material material) {
        ...
        int j = MathHelper.d((float) FastMath.floor(d0));
        int k = (int) FastMath.floor(this.locZ);
    ...
    public void f(NBTTagCompound nbttagcompound) {
        try {
            ...
            } //Im not secure with that. if you have errors, delete this.
    ...
    public boolean inBlock() {
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            int j = (int) FastMath.floor(this.locX + (double) f);
            int k = (int) FastMath.floor(this.locY + (double) this.getHeadHeight() + (double) f1);
            int l = (int) FastMath.floor(this.locZ + (double) f2);
    ...
    private final ThreadingManager.TaskQueueWorker tpQueue = ThreadingManager.createTaskQueue();
    public void b(int i) {
        this.tpQueue.queueTask(() -> {
            try {
                b_Async(i);
            } catch (Exception e) {
                MinecraftServer.getLogger().error("Error found in class Entity at changeDimension section.");
                MinecraftServer.getLogger().error("Thread " + Thread.currentThread().getName() + " encountered an exception: " + e.getMessage(), e);
            }
        });
    }

    public void b_Async(int i) {
        if (!this.world.isStatic && !this.dead) {
            if (!(this.getBukkitEntity() instanceof Player)) return;
    ...
    public void a(CrashReportSystemDetails crashreportsystemdetails) {
        ...
        crashreportsystemdetails.a("Entity\'s Block location", CrashReportSystemDetails.a((int) FastMath.floor(this.locX), (int) FastMath.floor(this.locY), (int) FastMath.floor(this.locZ)));
    ...
    public void setRemovalState(boolean state) {
        needsRemoved = state;
    }
}
