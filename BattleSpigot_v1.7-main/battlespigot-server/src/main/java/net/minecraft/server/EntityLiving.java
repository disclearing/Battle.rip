...
public abstract class EntityLiving extends Entity {
    ...
    public EntityLiving(World world) {
        ...
        this.k = true;
        this.aL = (float) (FastMath.random() + 1.0D) * 0.01F;
        this.setPosition(this.locX, this.locY, this.locZ);
        this.aK = (float) FastMath.random() * 12398.0F;
    ...
    protected void a(double d0, boolean flag) {
        ...
        if (flag && this.fallDistance > 0.0F) {
            ...
            int j = (int) FastMath.floor(this.locY - 0.20000000298023224D - (double) this.height);
    ...
    protected void aO() {
        for (Map.Entry<Integer, MobEffect> next : Sets.newHashSet(this.effects.entrySet())) {
    ...
    public boolean damageEntity(DamageSource damagesource, float f) {
        ...
        if (this.isInvulnerable()) {
            ...
            if (this.getHealth() <= 0.0F) {
                ...
                this.aF = 1.5F;
                boolean flag = true;
                boolean applyNoDamageTicks = true;

                //Koru start
                //TODO: Invisibility tick fix
                if(lastDamageSource != null && lastDamageSource != damagesource) {
                    if((damagesource instanceof EntityDamageSourceIndirect)){
                        ...
                        if(sourceIndirect.i() instanceof EntityArrow){
                            applyNoDamageTicks = false;
                        }else if(sourceIndirect.i() instanceof EntityFishingHook){
                            if(lastDamageSource instanceof EntityDamageSourceIndirect){
                                EntityDamageSourceIndirect sourceIndirect2 = (EntityDamageSourceIndirect) damagesource;
                                if(!(sourceIndirect2.i() instanceof EntityFishingHook)) {
                                    applyNoDamageTicks = false;
                                }
                            }
                        }
                    } else if(lastDamageSource.o() && (damagesource instanceof EntityDamageSource) && !this.hasEffect(MobEffectList.FIRE_RESISTANCE)){
                        applyNoDamageTicks = false;
                    }

                    /*if(lastDamageSource.o() && (damagesource instanceof EntityDamageSource)){
                        applyNoDamageTicks = false;
                    }*/

                    if (damagesource == DamageSource.DROWN || damagesource == DamageSource.FALLING_BLOCK) {
                        applyNoDamageTicks = false;
                    }
                }
                //Koru end

                if (applyNoDamageTicks && (float) this.noDamageTicks > (float) this.maxNoDamageTicks / 2.0F) {
                    return false;
    ...
    public void a(ItemStack itemstack) {
        ...
        for (int i = 0; i < 5; ++i) {
            Vec3D vec3d = Vec3D.a(((double) this.random.nextFloat() - 0.5D) * 0.1D, FastMath.random() * 0.1D + 0.1D, 0.0D);
    ...
    public void a(DamageSource damageSource, float f, double d0, double d1) {
        if (this.random.nextDouble() >= this.getAttributeInstance(GenericAttributes.c).getValue()) {
            ...
            // one point pinga start
            if (damageSource instanceof EntityDamageSource && profile.isOnePointSevent()) {
                Entity damager = damageSource.getEntity();
                double range = FastMath.hypot(damager.getBukkitEntity().getLocation().getX() - getBukkitEntity().getLocation().getX(), damager.getBukkitEntity().getLocation().getZ() - getBukkitEntity().getLocation().getZ());

                if (range > 2.7) {
                    if (isSprinting() && damager.isSprinting()) {
                        final org.bukkit.util.Vector playerVector = this.getBukkitEntity().getLocation().getDirection();
                        final org.bukkit.util.Vector entityVector = damager.getBukkitEntity().getLocation().getDirection();
                        playerVector.setY(0.0f);
                        entityVector.setY(0.0f);
                        final double degrees = playerVector.angle(entityVector);
                        if (FastMath.abs(degrees) < 1.5) {
                            //magnitude -= (onGround ? 0.15D : 0.22D);
                            this.al = true;
                            ...
                            this.motX -= d0 / magnitude * (profile.getHorizontal() / 4 - 0.1016);
                            this.motY += profile.getVertical();
                            this.motZ -= d1 / magnitude * (profile.getHorizontal() / 4 - 0.1016);
    ...
    protected void b(float f) {
        ...
        if (i > 0) {
            ...
            int k = (int) FastMath.floor(this.locY - 0.20000000298023224D - (double) this.height);
    ...
    protected boolean d(final DamageSource damagesource, float f) { // void -> boolean, add final
        if (!this.isInvulnerable()) {
            ...
            Function<Double, Double> armor = f12 -> -(f12 - EntityLiving.this.applyArmorModifier(damagesource, f12.floatValue()));
            ...
            Function<Double, Double> absorption = f14 -> -(FastMath.max(f14 - FastMath.max(f14 - EntityLiving.this.getAbsorptionHearts(), 0.0F), 0.0F));
    ...
    public void e(float f, float f1) {
        ...
        if (this.M() && (!(this instanceof EntityHuman) || !((EntityHuman) this).abilities.isFlying)) {
            d0 = this.locY;
            float f2 = (float) EnchantmentManager.getDepthStriderEnchantmentLevel(this);
            float f3 = 0.8F;
            if (f2 > 3.0F) {
                f2 = 3.0F;
            }
            ...
            if (this.onGround) {
                f2 = this.world.getType((int) FastMath.floor(this.locX), (int) FastMath.floor(this.boundingBox.b) - 1, (int) FastMath.floor(this.locZ)).frictionFactor * 0.91F;
            }
        ...
        float f6 = (float) (FastMath.sqrt(d0 * d0 + d1 * d1) * 4.0F);
    ...
    public Vec3D j(float f) {
        ...
        if (f == 1.0F) {
            f1 = (float) FastMath.cos(-this.yaw * 0.017453292F - 3.1415927F);
            f2 = (float) FastMath.sin(-this.yaw * 0.017453292F - 3.1415927F);
            f3 = (float) -FastMath.cos(-this.pitch * 0.017453292F);
            f4 = (float) FastMath.sin(-this.pitch * 0.017453292F);
            return Vec3D.a(f2 * f3, f4, f1 * f3);
        } else {
            f1 = this.lastPitch + (this.pitch - this.lastPitch) * f;
            f2 = this.lastYaw + (this.yaw - this.lastYaw) * f;
            f3 = (float) FastMath.cos(-f2 * 0.017453292F - 3.1415927F);
            f4 = (float) FastMath.sin(-f2 * 0.017453292F - 3.1415927F);
