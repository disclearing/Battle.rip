...
import dev.nulledcode.spigot.command.WorldStatsCommand;
import dev.nulledcode.spigot.util.pathsearch.PathSearchThrottlerThread;
...
public class ThreadingManager {
    ...
    private ScheduledExecutorService timerService = Executors.newScheduledThreadPool(1, new NamePriorityThreadFactory(Thread.NORM_PRIORITY + 2, "BattleSpigot_TimerService"));
    ...
    public ThreadingManager() {
        ...
        this.cachedThreadPoolFactory = new NamePriorityThreadFactory(Thread.currentThread().getPriority() - 1, "BattleSpigot_Async-Executor").setLogThreads(true).setDaemon(true);
    ...
    public void shutdown() {
        ...
        while((this.nbtFiles.isActive()) && !this.cachedThreadPool.isTerminated()) {
            try {
                ...
                log.warn("BattleSpigot is still waiting for NBT files to be written to disk. " + this.nbtFiles.getTaskCount() + " to go...");
        ...
        if(!this.cachedThreadPool.isTerminated()) {
            ...
            if(SpigotConfig.logRemainingAsyncThreadsDuringShutdown && this.cachedThreadPoolFactory.getActiveCount() > 0) {
                log.warn("BattleSpigot is still waiting for " + this.cachedThreadPoolFactory.getActiveCount() + " async threads to finish.");
